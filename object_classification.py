...
def parse_opt():
    ...
    parser.add_argument(
        '--weights', default=get_py_path() + 'wi_required/object_weight_file/best.onnx', help='Path of the weight file.', type=str)
    parser.add_argument(
        '--dest_path', default= get_py_path() + 'wi_required/WI_Folder/', help='Provide Destination Path to store the Output of Classificated Images.', type=str)
    ...
    parser.add_argument('--pet_loc', help='Provide path to save Pet Classificated images', default=get_py_path() + 'wi_required/WI_Folder/')
    parser.add_argument('--color_class', help='To do color classification', default= False, type=bool)
    parser.add_argument('--color_loc', help='Provide path to save Pet Classificated images', default=get_py_path() + 'wi_required/WI_Folder/')
    parser.add_argument('--save_csv', help='To save data into CSV file', default= False, type = bool)
    parser.add_argument('--csv_loc', default=get_py_path() + 'wi_required/WI_Folder/', help= 'Provide path to save CSV file to.', type = str)
        ...
        if label_path in label_path_arr:
            ...
            with open(label_dir + label_path) as file_:
                ...
                for line in lines:
                    ...
                    if pc == True:
                        if i == 0:
                            pet_call_process = multiprocessing.Process(target=call_pet, args=[str(crop_store + w_class[0] + '/' + x[0] + '_' + str(i) + '_' + str(counter[i]) + '.jpg'), pl, sc])
                    ...
                    if s_csv == True:
                        try:
                            with open(csv_loc + 'waste_data.csv', 'a+', encoding='UTF8', newline='') as f:
                                #For csv Data collection
                                csv_row = str(src_path + f) + ' ' + str(w_class[i]) + ' ' + str(i) + ' ' + str(x_cen) + ',' + str(y_cen) + ',' + str(wi) + ',' + str(hi)\
                                    + ' ' + str(counter[i])
                                writer = csv.writer(f)
...
def call_color(src_path, dest_path, sc):
    os_pet_command = 'python3 ' + get_py_path() + 'color_classification.py --src_path ' + src_path + ' --dest_path ' + dest_path + ' --crop_save ' + str(sc)
    os.system(os_pet_command)

def call_pet(src_path, dest_path, sc):
    os_pet_command = 'python3 ' + get_py_path() + 'pet_classification.py --src_path ' + src_path + ' --dest_path ' + dest_path + ' --crop_save ' + str(sc)
    os.system(os_pet_command)
